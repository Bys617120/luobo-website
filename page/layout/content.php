...
<head>
    ...
    <link rel="stylesheet" href="/assets/style/main.css?r=<?= time(); ?>">
...
<div class="container">
    <section id="content">
        ...
        <h2 class="sub-title"><?= $pageInfo['sub-title']; ?> |
            写于 <?= date('Y年n月j日', strtotime($pageInfo['time'])); ?></h2>
		
		<?php include('page/' . $page . '.php'); ?>
		<?php if ($page != 'dir' || ($search ?? '') != '') { ?>
            <h2 style="text-align:center;"><a href="javascript:history.go(-1)"><i class="iconfont icon-back"></i> 返回</a>
            </h2>
		<?php } ?>
    </section>
</div>
